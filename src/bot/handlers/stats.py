...
async def stats_total_command(update: Update, context: ContextTypes.DEFAULT_TYPE, user_manager: UserManager):
    ...
    try:
        await show_total_stats(update, stats_manager)
...
async def stats_today_command(update: Update, context: ContextTypes.DEFAULT_TYPE, user_manager: UserManager):
    ...
    try:
        await show_daily_stats(update, stats_manager, date.today())
    except Exception as e:
...
async def stats_users_total_command(update: Update, context: ContextTypes.DEFAULT_TYPE, user_manager: UserManager):
    ...
    try:
        await show_users_menu_stats(update, stats_manager, user_manager)
    except Exception as e:
...
async def stats_users_today_command(update: Update, context: ContextTypes.DEFAULT_TYPE, user_manager: UserManager):
    ...
    try:
        await show_users_menu_daily_stats(update, stats_manager, user_manager, date.today())
    except Exception as e:
...
async def stats_user_command(update: Update, context: ContextTypes.DEFAULT_TYPE, user_manager: UserManager):
    ...
    user_id = args[0]
    try:
        await show_user_stats(update, stats_manager, user_id)
    except Exception as e:
        logger.error(f"显示指定用户统计时出错: {str(e)}")
        await update.message.reply_text(f"显示统计数据时出错: {str(e)}")

async def show_total_stats(update: Update, stats_manager: UserStatsManager):
    """显示总体统计信息
...
async def show_daily_stats(update: Update, stats_manager: UserStatsManager, day: date):
    """显示每日统计信息
...
async def show_user_stats(update: Update, stats_manager: UserStatsManager, user_id: str):
    """显示用户统计信息
...
async def show_users_menu_stats(update: Update, stats_manager: UserStatsManager, user_manager: UserManager):
    """显示所有用户的菜单使用详情（总体统计）
...
async def show_users_menu_daily_stats(update: Update, stats_manager: UserStatsManager, user_manager: UserManager, day: date):
    """显示所有用户的菜单使用详情（每日统计）
...
def register_stats_commands(command_registry: CommandRegistry) -> None:
    """注册统计相关命令
    ...
    command_registry.register_command(
        CommandPlugin(
            ...
            handler=stats_users_total_command,
            category=CommandCategory.STATS,
            required_role=UserRole.ADMIN,
            is_visible=True
        )
    ...
    command_registry.register_command(
        CommandPlugin(
            ...
            handler=stats_users_today_command,
            category=CommandCategory.STATS,
            required_role=UserRole.ADMIN,
            is_visible=True
        )
